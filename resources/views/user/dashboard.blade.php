        ...
        <div class="container mx-auto p-6">
                ...
                <div class="flex flex-col md:flex-row items-center gap-6">
                    ...
                    <div class="flex gap-6">
                        <div class="text-center">
                            <p class="text-2xl font-bold text-green-600">{{ $totalXp ?? 0 }}</p>
                        ...
                        <div class="text-center">
                            <p class="text-2xl font-bold text-blue-600">#{{ $totalLevel ?? 1 }}</p>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div class="bg-white rounded-xl shadow-md p-4 text-center border-l-4 border-blue-500">
                    ...
                    <p class="text-xl font-bold text-gray-800">{{ $totalPickups ?? 0 }}</p>
                </div>
                <div class="bg-white rounded-xl shadow-md p-4 text-center border-l-4 border-yellow-500">
                    ...
                    <p class="text-xl font-bold text-yellow-600">{{ $pendingPickups ?? 0 }}</p>
                </div>
                <div class="bg-white rounded-xl shadow-md p-4 text-center border-l-4 border-green-500">
                    ...
                    <p class="text-xl font-bold text-gray-800">{{ $totalTransactions ?? 0 }}</p>
                </div>
                <div class="bg-white rounded-xl shadow-md p-4 text-center border-l-4 border-purple-500">
                    ...
                    <p class="text-xl font-bold text-purple-600">{{ $unreadNotifications ?? 0 }}</p>
                </div>
            </div>
                        ...
                        @forelse($recentPickups ?? [] as $pickup)
                            <div class="flex justify-between items-center border-b pb-2">
                                <div>
                                    ...
                                    @if ($pickup->weight)
                                        <p class="text-sm text-gray-600">{{ $pickup->weight }} kg</p>
                                    @endif
                                </div>
                                <span
                        ...
                        @forelse($recentTransactions ?? [] as $transaction)
                            <div class="flex justify-between items-center border-b pb-2">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600">{{ $transaction->description ?? 'Transaksi' }}</p>
                                </div>
                                <div class="text-right">
                                    <p
                                        class="font-bold {{ $transaction->type == 'earn' ? 'text-green-600' : 'text-red-600' }}">
                                        {{ $transaction->type == 'earn' ? '+' : '-' }}
                                        {{ number_format($transaction->points ?? 0, 0, ',', '.') }} poin
                                    </p>
                                    <span
                                        class="text-xs px-2 py-1 rounded-full 
